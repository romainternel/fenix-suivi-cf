...
- **Cache-busting** : chaque déploiement incrémente `?v=N` sur les **8** balises `<link>`/`<script>` du projet (css/style.css + les 7 fichiers `js/*.js`) — jamais sur les CDN externes. Version actuelle : **v254**.
...
|---|---|---|
| `match_data` | Une ligne par action de jeu (21 colonnes, cf. `COLS` dans le HTML) | Oui |
...
**Fonctionnel et en production (v254) :**
- [x] Import Excel → Supabase (remplacement complet des 4 tables de données)
- [x] Dashboard, page Analyse (5 onglets internes : Résumé/Timeline/Enclenchements/Gardien/Chat IA)
