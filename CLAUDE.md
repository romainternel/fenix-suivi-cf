...
- **Cache-busting** : chaque déploiement incrémente `?v=N` sur les **9** balises `<link>`/`<script>` du projet (css/style.css + les 8 fichiers `js/*.js`) — jamais sur les CDN externes. Version actuelle : **v289**.
...
- [x] Thème sombre (v287, phase 1/plusieurs) : bascule manuelle (bouton dans le header, `toggleAppTheme()`, persistée `localStorage.fenix-theme`), appliquée pour l'instant **uniquement au Dashboard** (fond de page, cartes équipe, barre matchs, tableau joueurs) — les autres pages restent inchangées même bascule activée, migration prévue page par page à la demande de Romain suite à un comparatif de design avec une autre app du club. Tableau joueurs du Dashboard : en-tête collant + chiffres tabulaires (amélioration permanente, les 2 thèmes)
