...
```
FENIX-HANDBALL-CF-SUIVI.html   Fichier principal (3857 lignes) : HTML de toutes les pages,
...
docs/                           Documentation du workflow BMAD (voir §9) :
  ...
  stories/STORY-N-*.md           48 stories numérotées en continu (STORY-01 à STORY-48) + 3 fichiers BACKLOG*.md
...
.gitignore                        node_modules/, supabase/.temp/, PHOTO JOUEURS/, .claude/, .playwright-mcp/, *.xlsm, *.xlsx
```
...
- **Cache-busting** : chaque déploiement incrémente `?v=N` sur les **9** balises `<link>`/`<script>` du projet (css/style.css + les 8 fichiers `js/*.js`) — jamais sur les CDN externes. Version actuelle : **v310**.
...
`PLAYER_PHOTOS` : objet statique `{ nomCanonique: { portrait, corps } }`, maintenu à la main, **volontairement séparé de `JOUEURS_TERRAIN`** (qui est recréé à chaque import). 18 des 22 joueurs actuels ont une photo (Roman.L, Yoran.C, Zacharie.D et Roméo.G n'en ont pas encore — repli initiales). Fichiers en `assets/photos/`, WebP, préchargés au boot du script.
...
**Fonctionnel et en production (v310) :**
...
**v310** : Chat IA de la page Analyse (onglet "Notes & Outils") **supprimé** à la demande de Romain ("Chat ia on le supprime") — feature rule-based jamais priorisée pour une connexion à l'API Claude (cf. historique). Retiré : markup (`.chat-suggestions`/`.chat-container`, `FENIX-HANDBALL-CF-SUIVI.html`), fonctions `sendChatMessage()`/`_sendChatSuggestion()`/`generateChatResponse()` + variable `chatHistory` (`js/page-analyse.js`), classes `.chat-*` + variante `[data-theme="dark"]` (`css/style.css`). L'onglet "Notes & Outils" ne contient plus que la note libre du coach (`#coach-analyse`). `computeSuperiorites()` reste utilisée par le bloc "Résumé du match" (`generateResume3Points`), seul appelant restant.

**Dernier audit complet de régression : 2026-09-23 (v310), verdict RAS** — cf. `docs/regression/audit-complet-2026-09-23.md`
...
- **`enclenchement` (colonne Excel legacy, `COLS` index 9)** : ne sert plus à la classification tactique principale (rôle repris par `intention_attaque`), mais reste activement lue à 2 endroits (drill-down niveau 2, colonne avancée) — ne pas retirer du schéma ni traiter comme morte.
