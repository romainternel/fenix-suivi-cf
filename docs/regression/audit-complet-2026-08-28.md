...
### 🟡 Observation — Données réelles de la saison 2025-2026 : "Intention attaque" et "Temps de jeu" vides
Sur les données réelles déjà chargées dans le navigateur (19 matchs, saison 2025-2026), deux colonnes sont systématiquement vides, quel que soit le match :
- **Familles d'enclenchement (I6)** : 100% des possessions tombent en "Non classifié" sur tous les matchs testés (`J01 FENIX-AIX`, `J05 MHB-FENIX`). Le drill-down confirme que la colonne `Intention attaque` du fichier Excel est vide pour ces lignes (`Non défini`, 60/60 possessions).
- **Temps de jeu (Dashboard, colonnes TPS JEU/MOY TJ/MATCHS TJ)** : affiche `0 min`/`null min`/`0` pour tous les joueurs sur les données réelles, alors que `TEMPS_JEU` contient bien 23 entrées joueur en mémoire — signe d'un problème de correspondance entre les clés stockées et celles attendues par l'affichage plutôt que d'une absence totale de données.

**Ce n'est pas une régression du cycle STORY-12→19** : aucune des 8 stories n'a touché la classification (`getEncFamille()`, `ENC_FAMILLE_MAP`) ni le calcul de temps de jeu (`getTJData`, `TEMPS_JEU`). Confirmé par contre-test : en réimportant le fichier de test `ESSAI IA STAT.xlsm` (§2, C3), les deux problèmes disparaissent complètement — classification correcte (aucun message "non classifiés"), temps de jeu affiché normalement (`60 min`/`30 min`/`2` matchs). Le code fonctionne ; c'est le fichier Excel réel de la saison en cours qui semble ne pas avoir ces deux colonnes renseignées dans le format attendu (probablement antérieur à la migration `Intention attaque`/`Tableau_MATCH` documentée en mémoire longue durée, ou un souci de correspondance de noms). **À signaler à Romain** pour vérification de son fichier Excel réel — hors périmètre de cet audit de régression (qui porte sur le code, pas sur le contenu des données).
